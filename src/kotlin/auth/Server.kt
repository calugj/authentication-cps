    ...
    override fun operate() {
        ...
        if(received.payload[0].contentEquals(AUTH)) {
            if(step == 0 && getInbound().size >= 1) {
                if(received.source.contentEquals(received.payload[1])) {
                    ...
                    val M2 = Message(ID, received.source, mutableListOf(AUTH, C1, r1))
                    sendMessage(M2)
                ...
                val decryption = decrypt(received.payload[1], k1)
                ...
                if(r1.contentEquals(retrieved_r1)) {
                    ...
                    val M4 = Message(ID, received.source, mutableListOf(AUTH, encryption))
                    sendMessage(M4)
                    ...
                    if(element[0].contentEquals(received.source)) {
        ...
        } else if(received.payload[0].contentEquals(DEAUTH)) {
            for(element in table)
                if(element[0].contentEquals(received.source)) {
                    table.remove(element)
                    break
                }
        } else if(received.payload[0].contentEquals(MESG) == isAuthenticated(received.source[0].toInt())){
            var encrypted = received.payload[1]
            ...
            val MESSAGE = Message(ID, received.source, mutableListOf(MESG, encrypted))
            sendMessage(MESSAGE)
        }
        removeInbound()
    }
